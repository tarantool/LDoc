...
local block_transform, blocks_to_rst, encode_code, span_transform, encode_backslash_escapes

local function final_cleanup(text)
	local function code_block(code)
		code = indent(indent(code))
		return ".. code-block:: lua \n " .. code
	end
	text = text:gsub("<pre>(.-)</pre>", code_block)
	return text
end
...
local function codeblocks(lines)
	...
	local function process_codeblock(lines)
		local raw = "\n" .. detab(encode_code(outdent(lines[1].line)))
		for i = 2,#lines do
...
function blocks_to_rst(lines, no_paragraphs)
	local out = {}
	local i = 1
	while i <= #lines do
		local line = lines[i]
